...
export function useCommitEvents() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      setProposedEvents((prev: ProposedEvent[]) =>
        prev.map(event => ({ ...event, status: 'committed' as const }))
      )
...
export function useConflictCheck() {
  ...
  const checkConflict = (event: ProposedEvent): boolean => {
    const allEvents = [...existingEvents, ...proposedEvents.filter(e => e.id !== event.id)]
