import type { CalendarEvent, ProposedEvent, Message, SchedulerState, CommitResponse } from './types'
...
export async function commitEvents(events: ProposedEvent[]): Promise<CommitResponse> {
...
export function parseProposedEvents(content: string): ProposedEvent[] | null {
  ...
  try {
    ...
    return parsed.map((event: ProposedEvent & { start: string; end: string }) => ({
      ...event,
      start: new Date(event.start),
      end: new Date(event.end),
    }))
